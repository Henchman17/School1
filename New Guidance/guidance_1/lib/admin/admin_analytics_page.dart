...
class _AdminAnalyticsPageState extends State<AdminAnalyticsPage> {
  ...
  Future<void> fetchAnalytics() async {
    ...
    try {
      ...
      final currentUser = AppConfig.supabase.auth.currentUser;
      if (currentUser == null) {
        setState(() {
          errorMessage = 'User not authenticated. Please log in to access analytics.';
          isLoading = false;
        });
        return;
      }
      ...
      final response = await AppConfig.supabase.functions.invoke(
        'api',
        body: {
      ...
      if (response.status == 200) {
        ...
        setState(() {
          errorMessage = 'Failed to load analytics: ${response.status}';
          isLoading = false;
        });
      }
    } catch (e) {
      ...
      if (e.toString().contains('AuthApiException')) {
        errorMsg = 'Authentication error. Please ensure you are logged in and try again.';
      }
      setState(() {
