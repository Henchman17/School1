  ...
  void _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      final data = {
        ...
        'grade_course_year_section': gradeController.text,
        'nickname': nicknameController.text,
        'ordinal_position': ordinalPositionController.text,
        'student_description': studentDescriptionController.text,
        'familial_description': familialDescriptionController.text,
        'strengths': strengthsController.text,
        'weaknesses': weaknessesController.text,
        'achievements': achievementsController.text,
        'best_work_person': bestWorkPersonController.text,
        'first_choice': firstChoiceController.text,
        'goals': goalsController.text,
        'contribution': contributionController.text,
        'talents_skills': talentsController.text,
        'home_problems': homeProblemsController.text,
        'school_problems': schoolProblemsController.text,
        'applicant_signature': applicantSignatureController.text,
        'signature_date': signatureDateController.text,
        'created_at': DateTime.now().toIso8601String(),
      };

      try {
        await AppConfig.supabase.from('routine_interviews').insert(data);
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Routine Interview Form Submitted Successfully')),
        );
