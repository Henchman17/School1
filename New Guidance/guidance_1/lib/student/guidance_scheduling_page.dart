...
class _GuidanceSchedulingPageState extends State<GuidanceSchedulingPage> {
  ...
  Future<void> _fetchAppointments() async {
    ...
    try {
      ...
      print('DEBUG: Retrieved ${response.length} appointments');

      // Print details of each appointment
      for (var i = 0; i < response.length; i++) {
        print('DEBUG: Appointment $i: ${response[i]}');
      }
  ...
  void _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        await AppConfig.supabase.from('appointments').insert(requestBody);
