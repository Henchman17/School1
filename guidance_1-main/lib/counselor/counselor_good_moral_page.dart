...
class CounselorGoodMoralRequestsPage extends StatefulWidget {
  final Map<String, dynamic>? userData;

  const CounselorGoodMoralRequestsPage({super.key, this.userData});

  @override
  State<CounselorGoodMoralRequestsPage> createState() => _CounselorGoodMoralRequestsPageState();
}

class _CounselorGoodMoralRequestsPageState extends State<CounselorGoodMoralRequestsPage> {
  ...
  Future<void> fetchGoodMoralRequests() async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('$apiBaseUrl/api/counselor/good-moral-requests?counselor_id=$counselorId'),
  ...
  Future<void> approveRequest(int requestId) async {
    try {
      ...
      final response = await http.put(
        Uri.parse('$apiBaseUrl/api/counselor/good-moral-requests/$requestId/approve?counselor_id=$counselorId'),
        headers: {'Content-Type': 'application/json'},
      );
  ...
  Future<void> rejectRequest(int requestId, String reason) async {
    try {
      final counselorId = widget.userData?['id'] ?? 0;
      final response = await http.put(
        Uri.parse('$apiBaseUrl/api/counselor/good-moral-requests/$requestId?counselor_id=$counselorId'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({'status': 'rejected', 'counselor_notes': reason}),
      );
