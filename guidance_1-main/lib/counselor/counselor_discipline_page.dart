...
class CounselorDisciplinePage extends StatefulWidget {
  final Map<String, dynamic>? userData;

  const CounselorDisciplinePage({super.key, this.userData});

  @override
  State<CounselorDisciplinePage> createState() => _CounselorDisciplinePageState();
}

class _CounselorDisciplinePageState extends State<CounselorDisciplinePage> {
  ...
  Future<void> _fetchPrograms() async {
    try {
      final response = await http.get(Uri.parse('$apiBaseUrl/api/courses'));
  ...
  Future<void> _fetchCases() async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('$apiBaseUrl/api/counselor/discipline-cases?counselor_id=$counselorId'),
  ...
  Future<void> _updateCaseStatus(int caseId, String status, String? actionTaken, String? notes) async {
    try {
      ...
      final response = await http.put(
        Uri.parse('$apiBaseUrl/api/counselor/discipline-cases/$caseId'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              _buildDetailRow('Course', caseData['program']),
              ...
              _buildDetailRow('Assigned Counselor', caseData['counselor_name'] ?? 'Unknown Counselor'),
              ...
              if (caseData['counselor_notes'] != null && caseData['counselor_notes'].isNotEmpty) ...[
                const Text('Counselor Notes:',
                  style: TextStyle(fontWeight: FontWeight.bold)),
                Padding(
                  padding: const EdgeInsets.only(top: 8, bottom: 16),
                  child: Text(caseData['counselor_notes']),
                ),
              ],
  ...
  void _showUpdateDialog(Map<String, dynamic> caseData) {
    ...
    String? selectedProgram = caseData['program'];
    ...
    final notesController = TextEditingController(text: caseData['counselor_notes']);
            ...
            ElevatedButton(
              onPressed: () {
                ...
                _updateDisciplineCase(caseData['id'], {
                  ...
                  'counselor_notes': notesController.text,
                });
  ...
  Future<void> _createDisciplineCase(Map<String, dynamic> caseData) async {
    try {
      ...
      final response = await http.post(
        Uri.parse('$apiBaseUrl/api/counselor/discipline-cases'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
          'counselor_id': counselorId,
          'status': 'open',
  ...
  Future<void> _deleteDisciplineCase(int caseId) async {
    try {
      ...
      final response = await http.delete(
        Uri.parse('$apiBaseUrl/api/counselor/discipline-cases/$caseId?counselor_id=$counselorId'),
        headers: {'Content-Type': 'application/json'},
      );
                        ...
                        children: [
                          ...
                          TextField(
                            ...
                            decoration: const InputDecoration(
                              ...
                              border: OutlineInputBorder(),
                              hintText: 'Leave empty to assign to current counselor',
                            ),
                          ),
                        ],
                      ),
                ...
                SingleChildScrollView(
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: 8),
                      _buildFilterChip('Grave Offenses', 'grave_offenses', Colors.red.shade600, false),
                    ],
                  ),
                ),
              ],
                                      ...
                                      Container(
                                        ...
                                        child: Column(
                                          children: [
                                            ...
                                            _buildInfoRow(Icons.school, 'Grade/Program', '${caseData['grade_level']} - ${caseData['program']} ${caseData['section']}'),
                                            const SizedBox(height: 8),
                                            _buildInfoRow(Icons.date_range, 'Incident Date', caseData['incident_date']?.toString().split('T')[0]),
                                            const SizedBox(height: 8),
                                            _buildInfoRow(Icons.location_on, 'Location', caseData['incident_location']),
                                            const SizedBox(height: 8),
                                            _buildInfoRow(Icons.warning, 'Severity', _getSeverityLabel(caseData['severity'])),
                                          ],
                                        ),
                                      ),
                                      const SizedBox(height: 12),
  ...
  String _getSeverityLabel(String? severity) {
    switch (severity) {
      case 'light_offenses':
        return 'Light Offenses';
      case 'less_grave_offenses':
        return 'Less Grave Offenses';
      case 'grave_offenses':
        return 'Grave Offenses';
      default:
        return 'Unknown';
    }
  }
