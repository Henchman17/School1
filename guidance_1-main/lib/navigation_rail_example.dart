...
class _NavigationRailExampleState extends State<NavigationRailExample> with SingleTickerProviderStateMixin {
  ...
  List<NavigationRailDestination> _buildAnimatedNavigationDestinations() {
    final isAdmin = _currentUser ?['role'] == 'admin';
    final isHead = _currentUser ?['role'] == 'head';
    ...
    destinations.addAll([
      NavigationRailDestination(
        icon: AnimatedContainer(
          ...
          child: Icon(
            Icons.settings,
            color: _selectedIndex == (isAdmin ? 9 : isHead ? 3 : 1) ? Colors.white : Colors.white70,
          ),
        ),
        label: Text('Settings'),
      ),
      NavigationRailDestination(
        icon: AnimatedContainer(
          ...
          child: Icon(
            Icons.logout,
            color: _selectedIndex == (isAdmin ? 10 : isHead ? 4 : 2) ? Colors.white : Colors.white70,
          ),
        ),
        label: Text('Logout'),
      ),
    ...
    if (isAdmin) {
      ...
      switch (index) {
        case 0: // Home
          setState(() => _selectedIndex = 0);
          break;
        case 1: // Head Dashboard
          _navigateToHeadDashboard();
          break;
        case 2: // Good Moral Requests
          _navigateToHeadGoodMoral();
          break;
        case 3: // Divider
          break;
        case 4: // Settings
  ...
  void _navigateToHeadDashboard() {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => HeadDashboardPage(userData: _currentUser ),
      ),
    );
  }

  void _navigateToHeadGoodMoral() {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => HeadGoodMoralPage(userData: _currentUser ),
      ),
    );
  }
}
